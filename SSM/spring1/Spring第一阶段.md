...
* <u>使用对象时，由主动new产生对象转换为由<mark>外部</mark>提供对象，此过程中对象创建控制权由程序转移到外部，此思想称为控制反转。</u>
...
* Spring技术对IOC思想进行了实现
* Spring提供了一个容器，称为<mark>IOC容器</mark>，用来充当IOC思想中的"外部"
...
* **<u>IOC:控制反转，控制反转的是对象的创建权</u>**
* **<u>DI:依赖注入，绑定对象与对象之间的依赖关系</u>**

(2)什么是IOC容器?

- **<u>Spring创建了一个容器用来存放所创建的对象，这个容器就叫IOC容器</u>**

(3)什么是Bean?

- **<u>容器中所存放的一个个对象就叫Bean或Bean对象</u>**
...
```java
其实就是反射机制
Class<?> clazz = Class.forName("com.itheima.dao.impl.BookDaoImpl");
Object bookDao = clazz.getDeclaredConstructor().newInstance();
```

> [!NOTE]
>
> <mark>**注意事项：bean定义时id属性在同一个上下文中(配置文件)不能重复**</mark>
...
> 在这里我们调用Service就不需要new对象了，可以直接到Bean容器中去取。
...
- **property标签**表示配置当前bean的属性，他嵌套在要注入依赖的bean标签中。
- **name属性**表示配置哪一个具体的属性。
- **ref属性**表示参照哪一个bean
...
> [!NOTE]
>
> <mark>**注意:配置中的两个bookDao的含义是不一样的**</mark>
>
...
public class App {
        public static void main(String[] args) {
               //3、获取IoC容器
                ...
                //通过set方法绑定
                BookService bookService=(BookService) ctx.getBean("bookService");
                // bookService容器中对应bean的id，或者是bean的别名。
                bookService.save();
...
> * 获取bean无论是通过id还是name获取，如果无法获取到，将抛出异常<mark>NoSuchBeanDefinitionException</mark>
...
2. bean在容器中是单例的，会不会产生 <mark>线程安全</mark>问题?
...
<img src="assets/1631529887695.png" alt="1631529887695" style="zoom: 67%;" />
...
* **bean是如何创建的**
* <mark>**实例化bean的三种方式，构造方法、静态工厂、实例工厂**</mark>

在讲解这三种创建方式之前，我们需要先确认一件事:

bean ==本质上就是对象== ，对象在new的时候会使用构造方法完成，那创建bean也是使用构造方法完成的。
...
运行程序，能执行成功,说明内部走的依然是构造函数,**能访问到类中的私有构造方法,显而易见Spring底层用的是<mark>反射</mark>**
...
public class AppForInstanceOrder {
    public static void main(String[] args) {
        //通过静态工厂创建对象
        OrderDao orderDao = OrderDaoFactory.getOrderDao();  //多态
        orderDao.save();
    }
}
```
...
(2)创建一个工厂类OrderDaoFactory并提供一个==普通方法==，注意此处和静态工厂的工厂类不一样的地方是方法不是静态方法
...
（4）静态工厂实例化不需要事先在IOC容器中配置静态工厂，而实例工厂需要事先在IOC容器中配置的原因是？

- **静态工厂**：

  - 静态工厂使用静态方法来创建Bean实例。

  - 不需要在IoC容器中配置静态工厂，因为它们的创建方法是静态的，可以直接通过==类名调用==。
  ...
  - 在配置文件中，可以直接使用<bean>标签来引用静态工厂方法：
      ```xml
       <bean id="myBean" class="com.example.MyStaticFactory" factory-method="createBean"/>
      <!--实际上发生了以下步骤：
      1. Spring 容器读取配置文件并开始初始化。
      2. Spring 知道要创建一个名为 `myBean` 的 Bean。
      3. Spring 查找 `com.example.MyStaticFactory` 类并加载它。
      4. Spring 调用 `MyStaticFactory.createBean()` 方法,因为是静态方法可以直接用类名来调用。
      5. `createBean()` 方法返回一个新的 `MyBean` 实例。
      6. Spring 将这个实例注册为 `myBean`。
      
      因此，`myBean` 实际上是通过调用 `MyStaticFactory.createBean()` 方法来创建的。这种方式允许我们将 Bean 的创建逻辑与 Bean 的使用分离，使代码更加清晰和模块化。在其他地方，我们可以通过 `myBean` 引用这个实例，例如在其他 Bean 的依赖注入中。-->
      ```

- **实例工厂**：

  - 实例工厂使用非静态方法来创建Bean实例。

  - 需要在IoC容器中配置实例工厂，因为它们的创建方法是非静态的，需要先创建工厂实例，然后通过工厂实例调用创建方法。
  ...
  - 在配置文件中，==首先配置实例工厂的Bean==，然后再引用工厂方法：
    ...
    <bean id="myBean" factory-bean="myInstanceFactory" factory-method="createBean"/>
    <!--
    实际上发生了以下步骤：
    1. Spring 容器读取配置文件并开始初始化。
    2. Spring 知道要创建一个名为 `myBean` 的 Bean。
    3. Spring 查找 `com.example.MyInstanceFactory` 类并加载它。
    4. Spring 创建 `myInstanceFactory` 实例，通过默认构造方法。
    5. 然后，Spring 调用 `myInstanceFactory` 实例的 `createBean()` 方法。
    6. `createBean()` 方法返回一个新的 `MyBean` 实例。
    7. Spring 将这个实例注册为 `myBean`。
    
    因此，`myBean` 实际上是通过调用 `MyInstanceFactory.createBean()` 方法来创建的。这种方式允许我们在 Bean 创建过程中添加自己的业务逻辑，从而更好地满足应用程序的需求。
    ```
...
##### 4.2.3.3 FactoryBean的使用(实例工厂的衍生)
...
> [!Note]
>
> 这里只需要指明FactoryBean的实现类，为要创建的bean起个id即可。因为FactoryBean由Spring所实现，它统一了创建bean的方法名。
...
方法一:`T getObject() throws Exception`：返回创建的 Bean 实例（具体实现类）。

方法二:`Class<?> getObjectType()`：返回创建的 Bean 类型（父类，接口）。

方法三:`boolean isSingleton()`：指示是否为单例 Bean。如果返回 `true`，则表示创建的 Bean 是单例，Spring 容器将缓存它。如果返       回 `false`，则表示每次请求都会创建一个新的 Bean 实例。
...
需要==注意==的一点是，构造方法在类中默认会提供，但是如果重写了构造方法，默认的就会消失，在使用的过程中需要注意，如果需要重写构造方法，最好把默认的构造方法也重写下。
...
- `init-method`属性用于指定在Bean初始化时要调用的方法。
- `destroy-method`属性用于指定在Bean销毁时要调用的方法。
...
**<u>Spring的IOC容器是运行在JVM中，运行main方法后，JVM启动，Spring加载配置文件生成IOC容器，从容器获取bean对象，然后调方法执行。main方法执行完后，JVM退出，这个时候IOC容器中的bean还没有来得及销毁就已经结束了，所以没有调用对应的destroy方法。</u>**
...
**不同点:**<u>close()是在调用的时候关闭，registerShutdownHook()是在JVM退出前调用关闭，所以close()一定要在程序最后执行，registerShutdownHook()则随意。</u>

分析上面的实现过程，会发现添加初始化和销毁方法，即需要编码也需要配置，实现起来步骤比较多也比较乱。而且每个程序员的书写风格都不同，所以方法命名也不同，在开发中会降低效率。

<u>**Spring提供了两个接口来完成生命周期的控制**，好处是可以不用再进行配置`init-method`和`destroy-method`</u>
...
* 思考:afterPropertiesSet和setBookDao谁先执行?
  ...
  * 重新运行AppForLifeCycle，打印结果如下:
    ...
    验证的结果和我们猜想的结果是一致的，所以初始化方法会在==类中属性设置之后执行==。

#### 4.3.5 bean生命周期小结

(1)关于Spring中对bean生命周期控制提供了两种方式:

* <u>在配置文件中的bean标签中添加`init-method`和`destroy-method`属性</u>
* <u>类实现`InitializingBean`与`DisposableBean`接口，这种方式了解下即可。</u>

(2)对于bean的生命周期控制在bean的整个生命周期中所处的位置如下:

* 初始化容器（容器创建就会执行，不需要调用bean）
  * 1.创建对象(内存分配)
  * 2.执行构造方法
  * 3.执行属性注入(set操作)
  * ==4.执行bean初始化方法==
...
其实从字面很好理解，`<bean id="bookService" class="com.itheima.service.impl.BookServiceImpl"></bean>`通过反射实例化对象，property表示类的字段属性，value和ref分别用于初始化（通过set方法）。
...
Spring默认使用无参构造方法创造bean，但是你这里提供了有参构造方法，所以默认的无参构造就不生效了。若你不告诉Spring用哪个构造方法就导致无法创建。<u>**Spring默认不会自动推断哪个有参构造方法应该被使用，尽管它可以通过反射找到所有的构造方法。**</u>
...
* <mark>当构造函数中方法的参数名发生变化后，配置文件中的name属性也需要跟着变。</mark>
* 这两块存在紧耦合，具体该如何解决?
...
<!--Spring 容器会尝试将某个 Bean 的属性与其他 Bean 的类型进行匹配。
    如果找到与属性类型相同的 Bean，就会自动将其注入到目标 Bean 中。-->
```
...
BookServiceImpl类中的字段名和id="bookDao"匹配。
...
1. **<u>自动装配用于引用类型依赖注入，不能对简单类型进行操作</u>**
2. <u>**使用按类型装配时（byType）必须保障容器中相同类型的bean唯一，推荐使用**</u>
3. **<u>使用按名称装配时（byName）必须保障容器中具有指定名称的bean，因变量名与配置耦合，不推荐使用</u>**
4. 自动装配优先级低于setter注入与构造器注入，同时出现时自动装配配置失效

### 5.4 集合注入

前面我们已经能完成引入数据类型和简单数据类型的注入，但是还有一种数据类型<mark>集合</mark>，集合中既可以装简单数据类型也可以装引用数据类型，对于集合，在Spring中该如何注入呢?
